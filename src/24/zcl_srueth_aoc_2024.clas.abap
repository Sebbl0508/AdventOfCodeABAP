CLASS zcl_srueth_aoc_2024 DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized" lang="de">Tag #05</p>
    "!
    METHODS day_05.
  ...
  PROTECTED SECTION.
    ...
    TYPES: BEGIN OF ts_day_05_input,
             page_duos TYPE ztt_srueth_aoc24_05_page_duo,
             updates   TYPE ztt_srueth_aoc_int4_2d,
           END OF ts_day_05_input.
    ...
    METHODS day_01_part_1
      IMPORTING it_list_1 TYPE ztt_srueth_aoc_int4
                it_list_2 TYPE ztt_srueth_aoc_int4.
    ...
    METHODS day_05_part_1
      IMPORTING
        is_input TYPE ts_day_05_input.
    METHODS day_05_part_2
      IMPORTING
        is_input TYPE ts_day_05_input.
...
METHOD day_05.
  DATA: lv_num1        TYPE i,
        ...
        lv_str1        TYPE string,
        lv_str2        TYPE string,
        lv_duos_parsed TYPE abap_bool VALUE abap_false,
        ls_input       TYPE ts_day_05_input.

  LOOP AT mt_input ASSIGNING FIELD-SYMBOL(<lv_line>).
    IF <lv_line> = space.
      lv_duos_parsed = abap_true.
      CONTINUE.
    ENDIF.

    IF lv_duos_parsed = abap_true.
      SPLIT <lv_line> AT ',' INTO TABLE DATA(lt_split).
      APPEND INITIAL LINE TO ls_input-updates ASSIGNING FIELD-SYMBOL(<lt_update>).
      LOOP AT lt_split ASSIGNING FIELD-SYMBOL(<lv_num>).
        lv_num1 = <lv_num>.
        APPEND lv_num1 TO <lt_update>.
      ENDLOOP.
    ELSE.
      SPLIT <lv_line> AT '|' INTO lv_str1 lv_str2.
      lv_num1 = lv_str1.
      lv_num2 = lv_str2.

      APPEND
        VALUE zst_srueth_aoc24_05_page_duo( page_x = lv_num1 page_y = lv_num2 )
        TO ls_input-page_duos.
    ENDIF.
  ENDLOOP.

  day_05_part_1( ls_input ).
  day_05_part_2( ls_input ).
ENDMETHOD.


METHOD day_05_part_1.

ENDMETHOD.


METHOD day_05_part_2.
ENDMETHOD.
